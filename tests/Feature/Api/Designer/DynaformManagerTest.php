...
class DynaformManagerTest extends ApiTestCase
{
    ...
    public function testCreateDynaform(Process $process, User $user): Dynaform
    {
        ...
        $data = [];
        //Post should have the parameter dyn_title, dyn_type
        ...
        $faker = Faker::create();
        $data['dyn_type'] =  $faker->sentence(1);
        $url = self::API_ROUTE . $process->PRO_UID . '/dynaform';
        $response = $this->api('POST', $url, $data);
        //validating the answer is an error
        $response->assertStatus(422);
        ...
        $data['dyn_type'] =  $faker->randomElement(Dynaform::TYPE);
     ...
     * @depends testCreateUser
     */
    public function testCreateBasedPmTable(Process $process, User $user): Dynaform
    {
        $this->auth($user->USR_USERNAME, self::DEFAULT_PASS);
     ...
     * @depends testCreateDynaform
     */
    public function testListDynaforms(Process $process, User $user): void
    {
        $this->auth($user->USR_USERNAME, self::DEFAULT_PASS);
        $structurePaginate = [
            'current_page',
    ...
    public function testGetDynaform(Process $process, User $user, Dynaform $Dynaform): void
    {
        ...
        //load Dynaform
        $url = self::API_ROUTE . $process->PRO_UID . '/dynaform/' . $Dynaform->OUT_DOC_UID;
        $response = $this->api('GET', $url);
        //Validate the answer is correct
        $response->assertStatus(200);
    ...
    public function testUpdateDynaform(Process $process, User $user, Dynaform $Dynaform): void
    {
        ...
        $data = [
            'dyn_title' => '',
            'dyn_description' => '',
            'out_doc_report_generator' => $faker->randomElement(Dynaform::DOC_REPORT_GENERATOR_TYPE),
            'out_doc_generate' => $faker->randomElement(Dynaform::DOC_GENERATE_TYPE),
            'dyn_type' => $faker->randomElement(Dynaform::DOC_TYPE)
        ];
        ...
        $data['dyn_description'] = $faker->sentence(2);
