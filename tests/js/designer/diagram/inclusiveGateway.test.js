...
describe('InclusiveGateway - Class', () => {
    let eGat

    beforeEach(() => {
        eGat = new Elements["InclusiveGateway"](
            {
    ...
    it('render() - Verify if use the library snap svg', () => {
        ...
        expect(mockGroup.mock.calls.length).toBe(2);
        expect(mockTrans.mock.calls.length).toBe(1);
        expect(mockPolyline.mock.calls.length).toBe(0);
    })
})