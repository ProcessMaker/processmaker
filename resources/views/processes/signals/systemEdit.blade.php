...
@section('content')
<div class="container" id="editSystemSignal">
  <div class="row">
    <div class="col-12">
      <nav>
        ...
        <div class="card card-body border-top-0 tab-pane p-3 fade show active" id="nav-detail" role="tabpanel" aria-labelledby="nav-detail-tab">
          <div class="modal-body">
            <div class="form-group">
              {!! Form::label('name', __('Signal Name'), [], false) !!}
              {!! Form::text('name', null, ['id' => 'name','class'=> 'form-control', 'v-model' =>
              'formData.name', 'v-bind:class' => '{\'form-control\':true, \'is-invalid\':errors.name}', 'required', 'aria-required' => 'true', 'disabled' => 'true']) !!}
              <div class="invalid-feedback" role="alert" v-for="name in errors.name">@{{name}}</div>
            </div>
            <div class="form-group">
              {!! Form::label('id', __('Signal ID'), [], false) !!}
              {!! Form::text('id', null, ['id' => 'id','class'=> 'form-control', 'v-model' =>
              'formData.id', 'v-bind:class' => '{\'form-control\':true, \'is-invalid\':errors.id}', 'required', 'aria-required' => 'true', 'disabled' => 'true']) !!}
              <small id="emailHelp" class="form-text text-muted"></small>
              <div class="invalid-feedback" role="alert" v-for="id in errors.id">@{{id}}</div>
            </div>
            <div class="form-group">
              {!! Form::textarea('detail', null, ['id' => 'detail', 'rows' => 4, 'class'=> 'form-control', 'v-bind:placeholder' => '$t("Description")',
          ...
          @isset($addons)
            ...
            @endforeach
          @endisset
          <div class="card-footer border-0 pb-0 px-0 text-right bg-transparent">
            {!! Form::button(__('Cancel'), ['class'=>'btn btn-outline-secondary', '@click' => 'onClose']) !!}
            {!! Form::button(__('Save'), ['class'=>'btn btn-secondary', '@click' => 'onUpdate', 'id'=>'saveSingal']) !!}
          </div>
        </div>
