			...
			<div class="flex-grow-1">
				<div class="container-fluid">
						...
						<template v-if="status">
							...
							@if(count($files) > 0 && !hasPackage('package-files'))
								<li class="nav-item">
									<a class="nav-link" id="files-tab" data-toggle="tab" href="#files" role="tab"
									   ...
									   @click="switchTab('files')">{{__('Files')}}</a>
								</li>
							@endif
							...
							@isset($addons)
								@foreach ($addons as $addon)
									<li class="nav-item">
										<a class="nav-link" id="{{$addon['id'] . '-tab'}}" data-toggle="tab"
										   href="{{'#' . $addon['id']}}"
					...
					<div class="tab-content" id="requestTabContent">
						<div class="tab-pane card card-body border-top-0 p-0" :class="{ active: activeErrors }"
							 id="errors" role="tabpanel"
							 aria-labelledby="errors-tab">
							<request-errors :errors="errorLogs"></request-errors>
						</div>
						<div class="tab-pane fade show card card-body border-top-0 p-0"
						...
						<div class="card card-body border-top-0 p-0"
							 v-bind:class="{ 'tab-pane':true, active: showSummary && !activePending}" id="summary"
							 role="tabpanel" aria-labelledby="summary-tab">
							<template v-if="showSummary">
								<template v-if="showScreenSummary">
									<div class="p-3">
										<vue-form-renderer ref="screen" :config="screenSummary.config"
														   v-model="dataSummary" :computed="screenSummary.computed"/>
									</div>
								</template>
