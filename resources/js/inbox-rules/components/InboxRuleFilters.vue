  ...
  <div>
    <div v-if="!savedSearchId && !task">
      <PMMessageScreen>
        <template #content>
          <img
            src="/img/funnel-fill-elements-blue.svg"
...
export default {
  ...
  methods: {
    ...
    addRequiredSavedSearchFilters(filter) {
      if (filter.filters) {
        const hasUserIdFilter = filter.filters.some((f) => f.subject.type === "Field" && f.subject.value === "user_id");
    ...
    onTaskListRendered() {
      if (this.columns.length <= 0 && this.defaultColumns.length <= 0) {
        const defaultColumns = this.$refs.taskList.tableHeaders;
