  ...
  <div>
    ...
    >
      <label
        for="custom-font"
        class="d-none"
      >{{ $t('Select which font to use throughout the system.') }}</label>
      <multiselect
        id="custom-font"
    ...
    <div class="d-flex group-button">
      ...
      <b-button
        ...
        @click="onAddToBundle"
      >
        <i class="fp-add-outlined" /> {{ $t('Add to Bundle') }}
      </b-button>
    ...
    >
      <div class="container text-center">
        ...
        <h3 class="display-6">
          {{ $t('Regenerating CSS Files') }}
        </h3>
        <p class="lead">
          {{ $t('Please wait while the files are generated. The screen will be updated when finished.') }}
        </p>
      </div>
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    placeholder(object, string) {
      const filename = _.get(object, "selectedFile");
