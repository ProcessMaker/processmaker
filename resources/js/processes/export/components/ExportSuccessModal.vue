  ...
  <div>
    ...
    >
      <template>
        <div class="export-successful">
          <h6 class="card-title export-type">
            <span class="font-weight-bold text-capitalize">{{
              processName
            }}</span>{{ $t(" was successfully exported.") }}
          </h6>
        </div>
        ...
        <ul class="pl-0">
          <li
            v-for="(value, key) in exportInfo.exported"
            :key="key"
          >
            <i class="fas fa-check-circle text-success" />{{ value.ids.length }}
        ...
        <template v-if="$root.includeAll === false">
          ...
          <ul class="pl-0">
            <li
              v-for="(value, key) in filterNonExported"
              :key="key"
            >
              <i class="fas fa-minus-circle" />{{ value.ids.length }}
              <span v-if="value.ids.length > 1">{{
                value.firstAsset.type_human_plural
              }}</span>
  ...
  computed: {
    filterNonExported() {
      return Object.entries(this.$root.ioState)
        .filter(([uuid, setting]) => setting.mode === "discard")
        .map(([uuid, setting]) => this.$root.manifest[uuid])
        .reduce((groups, asset) => {
