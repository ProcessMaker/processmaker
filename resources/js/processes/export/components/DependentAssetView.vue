<template>
  <div class="mb-2">
    ...
    <div>
      ...
      <div
        v-if="$root.includeAllByGroup[group.type]"
        class="mb-2"
      >
        <h6 class="mb-0 fw-semibold">
          {{ $t($root.operation) }} {{ $t('Status') }}:
          <b-badge
            pill
            variant="success"
          >
      ...
      <div class="mb-2">
        <b-link @click="returnToSummaryClick">
          {{ $t('Return to Summary') }}
        </b-link>
      </div>
    </div>
    <hr>
    <div
      v-for="(item, i) in group.items.filter(i => !i.hidden)"
      :key="i"
    >
        ...
        <b-card-text>
          <ul class="process-element-metadata">
            <li v-if="item.description">
              {{ $t('Description') }}: <span class="fw-semibold">{{ item.description }}</span>
            </li>
            <li>{{ $t('Categories') }}: <span class="fw-semibold">{{ item.categories }}</span></li>
            <li
              v-for="(attribute, i) in item.extraAttributes"
              :key="i"
            >
            ...
            <li>
              {{ $t('Translations') }}:
              <small
                v-for="language in item.translatedLanguages"
                class="mr-1 badge badge-pill badge-primary"
              >
                {{ language }}
              </small>
            </li>
