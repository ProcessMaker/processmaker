  ...
  <div>
    ...
    <div v-if="!disabled">
      ...
      <div v-for="configurable in optionsConfigurables">
        <h6
          v-if="configurable.startsWith('SECTION_TITLE:')"
          class="font-weight-bold mt-3"
          v-text="configurableLabel(configurable)"
        />
        <form-checkbox
          v-else-if="configurable !== 'SELF_SERVICE' || (configurable === 'SELF_SERVICE' && showAssignSelfService)"
          :key="configurable"
...
export default {
  ...
  computed: {
    ...
    assignments: {
      get() {
        const value = [];
        const users = this.assignedUserGetter ? this.assignedUserGetter.split(",") : [];
        const groups = this.assignedGroupGetter ? this.assignedGroupGetter.split(",") : [];
        value.users = users;
        value.groups = groups;
        return value;
      },
      set(value) {
        const assignedUsers = value.users.map((user) => (user.id ? user.id : user));
    ...
    optionsConfigurables() {
      const options = ["self_service", "rule_expression"];
  ...
  methods: {
    ...
    assignedUserSetter(id) {
      const value = this.formatIfById(id);
      this.$set(this.node, "assignedUsers", value);
    },
