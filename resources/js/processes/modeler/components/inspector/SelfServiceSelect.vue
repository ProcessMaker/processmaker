<template>
  <div>
    <div class="d-flex justify-content-between mb-3 align-items-center">
      ...
      <b-button
        :aria-label="$t('Add Self Service')"
        class="add-button rounded-0"
        variant="secondary"
        size="sm"
        @click="showCard = true"
      >
    ...
    >
      ...
      <div class="card-footer text-right p-2">
        ...
        <button
          ...
          @click="addSelfServiceAssignment()"
        >
          {{ $t('Add') }}
        </button>
      </div>
    ...
    <div class="pb-3">
      <div
        v-for="(items, itemIndex) in selfServiceAssignments"
        :key="itemIndex"
      >
        <div
          v-for="(assignment, assignmentIndex) in items"
          :key="assignmentIndex"
          class="row"
        >
          <div
            class="d-flex col-12 border-top self-service-assignments align-items-center py-2"
            :class="rowCss(assignmentIndex)"
          >
            ...
            <div class="col-1">
              <b-button
                class="p-0 text-dark delete-button"
                variant="link"
                :title="$t('Delete')"
                :aria-label="$t('Delete')"
                @click="showDeleteConfirmation(itemIndex, assignment.name, assignmentIndex)"
              >
