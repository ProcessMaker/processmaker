    ...
    <div class="row">
      <div class="col">
        <div class="card text-center">
          ...
          <b-alert
            v-if="importIsRunning"
            show
            variant="warning"
          >
            {{ $t('An import is currently in progress. Only one import can run at a time.') }}
          </b-alert>
          <div class="card-body">
            <div
              v-if="! importing && ! imported && ! importIsRunning"
              id="pre-import"
            >
...
export default {
  components: {
    DraggableFileUpload, EnterPasswordModal, ImportProcessModal, OldProcessImporter, ImportLog,
  },
  filters: {
    titleCase(value) {
  ...
  methods: {
    ...
    validateProcessTemplateFile(formData) {
      ProcessMaker.apiClient.post(
        "/templates/process/import/validation",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
