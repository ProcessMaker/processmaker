        ...
        <b-row align-v="start">
          <b-col :class="{ 'border-bottom': existingAssets.length }">
            <ul class="descriptions pl-3 ml-1">
              <li class="mb-1">
                <span class="fw-semibold">{{ $t("Import As New") }}</span>{{ willCreate() }}
              </li>
              <li
                v-if="userHasEditPermissions"
                class="mb-1"
              >
                <span class="fw-semibold">{{ $t("Update") }}</span>{{ willOverwrite() }}
              </li>
            </ul>
          </b-col>
        </b-row>
...
export default {
  ...
  computed: {
    title() {
      if (this.existingAssets.length > 0) {
        return this.$t("Import {{type}}: {{item}}", {
          type: this.existingAssets[0].typeHuman,
          item: this.processName,
        });
      }
    },
  },
  ...
  methods: {
    ...
    willCreate() {
      if (this.existingAssets.length > 0) {
        return this.$t(" will create a new {{type}} in this environment.", {
          type: this.existingAssets[0].typeHuman.toLowerCase(),
        });
      }
    },
    willOverwrite() {
      if (this.existingAssets.length > 0) {
        return this.$t(
          " will overwrite any assets tied to the current {{type}}. This may cause unintended side effects.",
          { type: this.existingAssets[0].typeHuman.toLowerCase() },
        );
      }
    },
    warningTitle(asset) {
      return this.$t("Caution: {{type}} Already Exists", {
        type: asset.typeHuman,
      });
    },
    helperText(asset) {
      ...
      if (asset.existingUpdatedAt && asset.importingUpdatedAt) {
        ...
        const importingUpdatedAt = new Date(
          `${asset.importingUpdatedAt.replace(" ", "T")}.000000Z`,
        );
    ...
    noUpdatePermissions(asset) {
      return this.$t(
        "You do not have permissions to update the existing {{type}} in this environment.",
        { type: asset.typeHuman.toLowerCase() },
      );
    },
  },
};
