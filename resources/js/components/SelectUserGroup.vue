  ...
  <div class="form-group">
    ...
    >
      ...
      <template #option="{ option, search, index }">
        <b-badge
          v-if="Object.hasOwn(option, 'active_tasks_count') && activeTasksCount"
          variant="secondary"
        ...
        const groupsPromise = Promise.all(
          value.groups.map((item) => {
            if (typeof item === "number" || typeof item === "string") {
              ...
              const id = this.unformatGroup(item.assignee);
  ...
  methods: {
    ...
    loadGroups(filter) {
      ProcessMaker.apiClient
        ...
        .then((response) => {
          const groups = response.data.data.map((item) => this.formatGroup(item));
    ...
    getOptionLabel(option, index) {
      ...
      const label = this.customLabel(option, index);
