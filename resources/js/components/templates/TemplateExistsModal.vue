        ...
        <b-row align-v="start">
          <b-col>
            <ul class="descriptions pl-3 ml-1">
              <li class="mb-1">
                <span class="fw-semibold">{{ $t('Save As New') }}</span>{{ $t(' will create a new template in this environment.') }}
              </li>
              <li class="mb-1">
                <span class="fw-semibold">{{ $t('Update') }}</span>{{ $t(' will overwrite any templates tied to the current process. This may cause unintended side effects.') }}
              </li>
            </ul>
          </b-col>
        </b-row>
