      ...
      handler() {
        ...
        this.$emit(
          ...
          this.content instanceof Array
            ? this.content.map((item) => item.id).join(",")
            : this.content
              ? this.content.id
              : "",
        );
  ...
  methods: {
    ...
    load(filter) {
      ProcessMaker.apiClient
        .get(
          `${this.apiList
          }?order_direction=asc&status=active${
    ...
    loadUncategorized() {
      return ProcessMaker.apiClient
        .get(
          `${this.apiList
          }?filter=Uncategorized&per_page=1&order_by=id&order_direction=ASC`,
        )
        .then((response) => {
    ...
    setUncategorizedDefault() {
      ...
      const uncategorizedCategoryIndex = this.content.findIndex((c) => c.id === this.uncategorizedCategory.id);
