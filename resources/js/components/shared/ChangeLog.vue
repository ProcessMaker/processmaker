<template>
  <ul class="process-element-metadata">
    <li
      v-for="(data, i) in findExistingData(newData)"
      :key="i"
      v-html="data"
    />
...
export default {
  ...
  methods: {
    findExistingData(data) {
      const foundExistingData = this.existingData.find((existing) => data.uuid === existing.uuid);
      ...
      if (!foundExistingData) {
        ...
        return this.generateChangeLog(dataFields, data, true, null);
      }
      ...
      const newData = Object.entries(data).filter(([key, _]) => {
        const found = dataFields.find((field) => field.value === key);
        if (found) {
      ...
      for (const [key, value] of Object.entries(newData)) {
        ...
        const foundExisting = existingData.find((field) => field[0] === key);
        ...
        if (foundExisting) {
          ...
          const item = this.generateChangeLog(dataFields, newData, isEqual, existingData);
          listItems.push(...item);
        }
      }
      return listItems;
    ...
    generateChangeLog(fields, newData, isEqual, existingData = null) {
      const list = [];
      for (const [key, value] of Object.entries(newData)) {
    ...
    formatChangeLogData(name, newValue, oldValue, isEqual) {
      let value;
      const label = name !== null ? `${this.$t(name)}: ` : null;
      let content;
      ...
      if (isEqual) {
        ...
        const existingValue = !_.isEmpty(oldValue) ? `<span class="fw-semibold">${oldValue}</span>` : null;
        const existingData = `${label !== null ? label + existingValue : existingValue} `;
        const newData = `${label !== null ? label + value : value}`;
