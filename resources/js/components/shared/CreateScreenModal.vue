<template>
    <div>
        <b-button v-if="!hideAddBtn" ref="createScreenModalBtn" v-b-modal.createScreen :aria-label="$t('Create Screen')" class="mb-3 mb-md-0 ml-md-2">
            <i class="fas fa-plus" /> {{ $t("Screen") }}
        </b-button>
        <modal id="createScreen" :ok-disabled="disabled" :title="$t('Create Screen')" @hidden="onClose" @ok.prevent="onSubmit">
            <template v-if="countCategories">
              ...
              <category-select
                v-model="categoryType ? category_type_id : formData.screen_category_id"
...
<template v-else>
    <div>
        {{ $t("Categories are required to create a screen") }}</div>
    <a class="btn btn-primary container mt-2" href="/designer/screens/categories">
                {{ $t("Add Category") }}
              </a>
</template>
      </modal>
