    ...
    <span>
        ...
        <b-modal v-model="showReassignment" size="md" centered title="Reassign to" v-cloak>
            ...
            <div slot="modal-footer">
                <b-button :disabled="selectedIndex < 0" @click="reassignUser"
                    class="btn btn-outline-success btn-sm text-uppercase">
                    REASSIGN</b-button>
                <b-button @click="cancelReassign" class="btn btn-success btn-sm text-uppercase">
                    CANCEL
                </b-button>
            </div>
        </b-modal>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            reassignUser() {
                if (this.selected) {
                    ProcessMaker.apiClient.put("tasks/" + this.task.id, {
