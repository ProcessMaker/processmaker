  ...
  methods: {
    ...
    assignedTo(delegations) {
      ...
      delegations.forEach(function(delegation, key) {
        if (
          delegation.user &&
          usedAvatar.includes(delegation.user.id) === false
        ) {
          ...
          if (key <= 4) {
            ...
            assignedTo += user.avatar
              ? '<img class="avatar-image-list avatar-circle-list" src="' +
                ...
                user.fullname +
                '"> '
              : '<button type="button" class="avatar-circle-list" title="' +
                user.fullname +
                '">' +
      ...
      if (count) {
        assignedTo +=
          '<button type="button" class="avatar-circle-list"><span class="avatar-initials-list">+' +
          count +
          "</span></button>";
      }
      return assignedTo;
