...
Scripts are executed inside docker containers that have access to the API. CircleCi automatically builds and installs the images for the qa servers.

Here's how to generate the SDK and test them locally for a specific language, in this case PHP. For best results, commands should be run in the vagrant box.
1. Edit your .env and specify the executor image to use (the image will be built in subsequent steps)

   `SCRIPTS_PHP_IMAGE=processmaker/pm4-docker-executor-php:dev-master`

1. Generate the OpenAPI docs

   `php artisan l5:generate`

1. Generate the SDK
   
   `php artisan bpm:sdk php /tmp/pm4-sdk-php`

1. Clone the executor repository
   
   `git clone https://github.com/ProcessMaker/pm4-docker-executor-php.git /tmp/pm4-docker-executor-php`

1. Copy the SDK into the executor folder. The folder name is important because it will download the SDK from github if the folder is not present

   `cp -r /tmp/pm4-sdk-php /tmp/pm4-docker-executor-php/src`

1. In the executor, run the build script

   `cd /tmp/pm4-docker-executor-php && ./build.sh`

   If everything worked, you should now have a `processmaker/pm4-docker-executor-php:dev-master` image available to docker
