...
use ProcessMaker\Facades\QueueService;
use ProcessMaker\Managers\Nayra\EntityRepositoryFactory;

class WorkflowNayraManager
{
     ...
     * @return Model|null
     */
    public function storeEntities(array $transactions, array $extraProperties = []): ?Model
    {
        $repositories = [];

        foreach ($transactions as $transaction) {
            $repository = EntityRepositoryFactory::createRepository($transaction['entity']);
            $properties = $transaction['properties'];

            if (!empty($extraProperties) && empty($repositories)) {
                $transaction['properties'] = array_merge($properties, $extraProperties);
            }

            $model = $repository->create($transaction);
